...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < offset; ++i)
        buf[i] = ' ';
    result = evaluate(player_cross, player_circle, games_count, NULL);
    printf(
        "\nCross wins: %*d/%*d (%.3f%%)\nCircle wins: %*d/%*d (%.3f%%)\nDraws: "
        "%*d/%*d (%.3f%%)",
...
#define write_games(out, current, total, total_len)                            \
    {                                                                          \
        char *null_char = out + sprintf(out, "\rGames: %*d/%*d (%.2f%%)",      \
                                        total_len, current, total_len, total,  \
